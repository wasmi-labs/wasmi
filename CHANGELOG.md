...
- Add `Memory::data_and_store_mut` API inspired by Wasmtime's API. (https://github.com/paritytech/wasmi/pull/462)

### Changed

- Update `wasmparser-nostd` dependency from `0.90.0` to `0.91.0`.
    - This improved performance of Wasm module compilation by ~10%.
- Update `wasmi_core` from `0.3.0` to `0.4.0`.
- Optimize execution of several Wasm float to int conversion instructions. (https://github.com/paritytech/wasmi/pull/439)
...
- Optimize `global.get` and `global.set` Wasm instruction execution. (https://github.com/paritytech/wasmi/pull/427)
- Optimize Wasm value stack emulation. (https://github.com/paritytech/wasmi/pull/459)

### Internal

- Add automated continuous benchmarking to `wasmi`. (https://github.com/paritytech/wasmi/pull/422)
    - This allows us to have a more consistent overview over the performance of `wasmi`.
- Update `criterion` benchmarking framework to version `0.4.0`.
...
- Enable more useful `clippy` lints for `wasmi` and `wasmi_core`. (https://github.com/paritytech/wasmi/pull/438)
