...
#[repr(transparent)]
pub struct DedupFuncType(GuardedEntity<EngineIdent, DedupFuncTypeIdx>);

impl DedupFuncType {
    /// Creates a new function signature reference.
    pub(super) fn from_inner(stored: GuardedEntity<EngineIdent, DedupFuncTypeIdx>) -> Self {
        Self(stored)
    }
...
impl FuncTypeRegistry {
    /// Creates a new [`FuncTypeRegistry`] using the given [`EngineIdent`].
    pub(crate) fn new(engine_idx: EngineIdent) -> Self {
        Self {
            engine_idx,
