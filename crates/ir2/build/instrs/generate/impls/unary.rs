...
use crate::instrs::{Context, UnaryOp};
use core::{fmt, fmt::Display};

pub struct DisplayUnaryOpImplsFile<'a> {
    ctx: &'a Context,
    indent: DisplayIndent,
}

impl<'a> DisplayUnaryOpImplsFile<'a> {
    pub fn new(ctx: &'a Context, indent: DisplayIndent) -> Self {
        Self { ctx, indent }
    }
}

impl Display for DisplayUnaryOpImplsFile<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let indent = self.indent;
        let unary_ops = DisplayUnaryOpImpls::new(&self.ctx.unary_ops, indent);
        emit!(f, indent =>
...
pub struct DisplayUnaryOpImpls<'a> {
    ops: &'a [UnaryOp],
    indent: DisplayIndent,
}

impl<'a> DisplayUnaryOpImpls<'a> {
...
impl Display for DisplayUnaryOpImpls<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
