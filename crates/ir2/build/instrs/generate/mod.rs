...
pub fn generate_ops(ctx: &Context) -> Result<(), io::Error> {
    ...
    generate_file("update_result.rs", DisplayUpdateOpResult::new(ctx, indent))?;
    generate_file(
        "impls/unary.rs",
    ...
    generate_file(
        "impls/cmp_branch.rs",
        DisplayCmpBranchOperatorImpls::new(&ctx.cmp_branch_ops, indent),
    )?;
    generate_file(
        "impls/binary.rs",
