...
impl VisitInputRegisters for Instruction {
    ...
    fn visit_input_registers(&mut self, mut f: impl FnMut(&mut Register)) {
        match self {
            ...
            Instruction::BranchI32Nand(instr) => instr.visit_input_registers(f),
            Instruction::BranchI32NandImm(instr) => instr.visit_input_registers(f),
            Instruction::BranchI32Nor(instr) => instr.visit_input_registers(f),
            Instruction::BranchI32NorImm(instr) => instr.visit_input_registers(f),
            Instruction::BranchI32Xnor(instr) => instr.visit_input_registers(f),
            Instruction::BranchI32XnorImm(instr) => instr.visit_input_registers(f),
