...
impl VisitInputRegisters for Instruction {
    ...
    fn visit_input_registers(&mut self, mut f: impl FnMut(&mut Register)) {
        match self {
            ...
            Instruction::I32EqAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32EqAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32NeAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32NeAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32LtSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32LtSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32LtUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32LtUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32GtSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32GtSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32GtUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32GtUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32LeSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32LeSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32LeUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32LeUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32GeSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32GeSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32GeUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32GeUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I64EqAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64EqAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64NeAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64NeAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64LtSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64LtSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64LtUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64LtUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64GtSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64GtSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64GtUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64GtUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64LeSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64LeSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64LeUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64LeUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64GeSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64GeSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64GeUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64GeUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F32EqAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32EqAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32NeAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32NeAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32LtAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32LtAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32LeAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32LeAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32GtAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32GtAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32GeAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32GeAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F64EqAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64EqAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64NeAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64NeAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64LtAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64LtAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64LeAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64LeAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64GtAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64GtAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64GeAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64GeAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I32AddAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32AddAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32SubAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32SubAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32MulAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32MulAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32DivSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32DivSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32DivUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32DivUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32RemSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32RemSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32RemUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32RemUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32AndAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32AndAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32OrAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32OrAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32XorAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32XorAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32ShlAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32ShlAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32ShrUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32ShrUAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32ShrSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32ShrSAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32RotlAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32RotlAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I32RotrAssign(instr) => instr.visit_input_registers(f),
            Instruction::I32RotrAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::I64AddAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64AddAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64SubAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64SubAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64MulAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64MulAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64DivSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64DivSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64DivUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64DivUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64RemSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64RemSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64RemUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64RemUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64AndAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64AndAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64OrAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64OrAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64XorAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64XorAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64ShlAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64ShlAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64ShrUAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64ShrUAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64ShrSAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64ShrSAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64RotlAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64RotlAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::I64RotrAssign(instr) => instr.visit_input_registers(f),
            Instruction::I64RotrAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F32AddAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32AddAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32SubAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32SubAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32MulAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32MulAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32DivAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32DivAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32MinAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32MinAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32MaxAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32MaxAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F32CopysignAssign(instr) => instr.visit_input_registers(f),
            Instruction::F32CopysignAssignImm(instr) => instr.visit_input_registers(f),
            Instruction::F64AddAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64AddAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64SubAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64SubAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64MulAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64MulAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64DivAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64DivAssignImm32(instr) => instr.visit_input_registers(f),
            Instruction::F64MinAssign(instr) => instr.visit_input_registers(f),
            Instruction::F64MinAssignImm32(instr) => instr.visit_input_registers(f),
