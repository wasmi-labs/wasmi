use crate::{core::UntypedVal, engine::TranslationError, ir::Local, Error};
use alloc::{
...
///
/// - Those constant values are identified by their associated [`Local`].
...
///   [`Local`] values refer to the equal constant values can be efficiently
///   done by comparing the [`Local`] indices without resolving to their
///   underlying constant values.
#[derive(Debug, Default)]
pub struct FuncLocalConsts {
    /// Mapping from constant [`UntypedVal`] values to [`Local`] indices.
    const2idx: BTreeMap<UntypedVal, Local>,
    /// Mapping from [`Local`] indices to constant [`UntypedVal`] values.
    idx2const: Vec<UntypedVal>,
    /// The [`Local`] index for the next allocated function local constant value.
    next_idx: i16,
}
...
impl FuncLocalConsts {
    ...
    /// The maximum index for [`Local`] referring to function local constant values.
    ///
    /// # Note
    ///
    ...
    /// The mininmum index for [`Local`] referring to function local constant values.
    ///
    /// # Note
    ///
    ...
    pub fn alloc(&mut self, value: UntypedVal) -> Result<Local, Error> {
        ...
        match self.const2idx.entry(value) {
            ...
            btree_map::Entry::Vacant(entry) => {
                let register = Local::from(self.next_idx);
                self.next_idx -= 1;
