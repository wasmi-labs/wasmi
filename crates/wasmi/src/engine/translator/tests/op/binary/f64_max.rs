...
fn reg_reg() {
    test_binary_local_reg(WASM_OP, Instruction::f64_max)
}

#[test]
#[cfg_attr(miri, ignore)]
fn reg_imm() {
    test_binary_local_imm32(WASM_OP, 1.0_f64, Instruction::f64_max)
}

#[test]
#[cfg_attr(miri, ignore)]
fn reg_imm_lhs() {
    test_binary_local_imm32_lhs_commutative(WASM_OP, 1.0_f64, Instruction::f64_max)
}
...
fn loc_neg_infinity() {
    ...
    test_binary_local_imm_with(WASM_OP, f64::NEG_INFINITY, expected).run()
}
...
fn loc_neg_infinity_lhs() {
    let expected = [Instruction::return_reg(0)];
    test_binary_local_imm_lhs_with(WASM_OP, f64::NEG_INFINITY, expected).run()
}
