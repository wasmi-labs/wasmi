...
fn test_init(ty: ValType) {
    ...
    TranslationTest::new(&wasm)
        .expect_func_instrs([
            Instruction::table_init(Local::from(0), Local::from(1), Local::from(2)),
...
fn test_init_exact16(ty: ValType, len: u32) {
    testcase_init_exact(ty, len)
        .expect_func_instrs([
            Instruction::table_init_exact(Local::from(0), Local::from(1), u32imm16(len)),
            Instruction::table_index(0),
            Instruction::elem_index(0),
            Instruction::Return,
        ])
...
fn test_init_exact(ty: ValType, len: u32) {
    testcase_init_exact(ty, len)
        .expect_func(
            ExpectedFunc::new([
                Instruction::table_init(Local::from(0), Local::from(1), Local::from(-1)),
                Instruction::table_index(0),
                Instruction::elem_index(0),
                Instruction::Return,
            ])
...
fn test_init_from16(ty: ValType, src: u32) {
    testcase_init_from(ty, src)
        .expect_func_instrs([
            Instruction::table_init_from(Local::from(0), u32imm16(src), Local::from(1)),
            Instruction::table_index(0),
            Instruction::elem_index(0),
            Instruction::Return,
        ])
...
fn test_init_to16(ty: ValType, dst: u64) {
    testcase_init_to(ty, dst)
        .expect_func_instrs([
            Instruction::table_init_to(u64imm16(dst), Local::from(0), Local::from(1)),
            Instruction::table_index(0),
            Instruction::elem_index(0),
            Instruction::Return,
        ])
...
fn test_init_to(ty: ValType, dst: u64) {
    testcase_init_to(ty, dst)
        .expect_func(
            ExpectedFunc::new([
                Instruction::table_init(Local::from(-1), Local::from(0), Local::from(1)),
                Instruction::table_index(0),
                Instruction::elem_index(0),
                Instruction::Return,
            ])
...
fn test_init_from_to16(ty: ValType, dst: u64, src: u32) {
    testcase_init_from_to(ty, dst, src)
        .expect_func_instrs([
            Instruction::table_init_from_to(u64imm16(dst), u32imm16(src), Local::from(0)),
            Instruction::table_index(0),
            Instruction::elem_index(0),
            Instruction::Return,
        ])
...
fn test_init_from_to(ty: ValType, dst: u64, src: u32) {
    testcase_init_from_to(ty, dst, src)
        .expect_func(
            ExpectedFunc::new([
                Instruction::table_init(Local::from(-1), Local::from(-2), Local::from(0)),
                Instruction::table_index(0),
                Instruction::elem_index(0),
                Instruction::Return,
            ])
...
fn test_init_to_exact16(ty: ValType, dst: u64, len: u32) {
    testcase_init_to_exact(ty, dst, len)
        .expect_func_instrs([
            Instruction::table_init_to_exact(u64imm16(dst), Local::from(0), u32imm16(len)),
            Instruction::table_index(0),
            Instruction::elem_index(0),
            Instruction::Return,
        ])
...
fn test_init_to_exact(ty: ValType, dst: u64, len: u32) {
    testcase_init_to_exact(ty, dst, len)
        .expect_func(
            ExpectedFunc::new([
                Instruction::table_init(Local::from(-1), Local::from(0), Local::from(-2)),
                Instruction::table_index(0),
                Instruction::elem_index(0),
                Instruction::Return,
            ])
...
fn test_init_from_exact16(ty: ValType, src: u32, len: u32) {
    testcase_init_from_exact(ty, src, len)
        .expect_func_instrs([
            Instruction::table_init_from_exact(Local::from(0), u32imm16(src), u32imm16(len)),
            Instruction::table_index(0),
            Instruction::elem_index(0),
            Instruction::Return,
        ])
...
fn test_init_from_to_exact(ty: ValType, dst: u64, src: u32, len: u32) {
    testcase_init_from_to_exact(ty, dst, src, len)
        .expect_func(
            ExpectedFunc::new([
                Instruction::table_init(Local::from(-1), Local::from(-2), Local::from(-3)),
                Instruction::table_index(0),
                Instruction::elem_index(0),
                Instruction::Return,
            ])
