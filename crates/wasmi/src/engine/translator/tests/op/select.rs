...
fn reg() {
    fn test_reg(kind: SelectKind, result_ty: ValType) {
        ...
        let condition = Local::from(0);
        let lhs = Local::from(1);
        let rhs = Local::from(2);
...
fn same_reg() {
    fn test_same_reg(kind: SelectKind, result_ty: ValType) {
        ...
        TranslationTest::new(&wasm)
            .expect_func_instrs([Instruction::return_reg(Local::from(1))])
            .run();
    }
    fn test_for(kind: SelectKind) {
...
fn same_imm() {
    ...
    where
        ...
        DisplayWasm<T>: Display,
    {
        let instrs = [Instruction::return_reg(Local::from(-1))];
