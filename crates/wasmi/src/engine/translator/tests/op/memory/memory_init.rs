...
fn init() {
    ...
    TranslationTest::from_wat(wasm)
        .expect_func_instrs([
            Instruction::memory_init(Reg::from_i16(0), Reg::from_i16(1), Reg::from_i16(2)),
...
fn test_copy_exact16(len: u32) {
    testcase_init_exact(len)
        .expect_func_instrs([
            Instruction::memory_init_exact(Reg::from_i16(0), Reg::from_i16(1), u32imm16(len)),
            Instruction::data_idx(0),
            Instruction::Return,
        ])
        .run()
...
fn test_copy_exact(len: u32) {
    testcase_init_exact(len)
        .expect_func(
            ExpectedFunc::new([
                Instruction::memory_init(Reg::from_i16(0), Reg::from_i16(1), Reg::from_i16(-1)),
                Instruction::data_idx(0),
                Instruction::Return,
            ])
            .consts([len]),
...
fn test_copy_from16(src: u32) {
    testcase_init_from(src)
        .expect_func_instrs([
            Instruction::memory_init_from(Reg::from_i16(0), u32imm16(src), Reg::from_i16(1)),
            Instruction::data_idx(0),
            Instruction::Return,
        ])
        .run()
...
fn test_copy_to16(dst: u32) {
    testcase_init_to(dst)
        .expect_func_instrs([
            Instruction::memory_init_to(u32imm16(dst), Reg::from_i16(0), Reg::from_i16(1)),
            Instruction::data_idx(0),
            Instruction::Return,
        ])
        .run()
...
fn test_copy_to(dst: u32) {
    testcase_init_to(dst)
        .expect_func(
            ExpectedFunc::new([
                Instruction::memory_init(Reg::from_i16(-1), Reg::from_i16(0), Reg::from_i16(1)),
                Instruction::data_idx(0),
                Instruction::Return,
            ])
            .consts([dst]),
...
fn test_copy_from_to16(dst: u32, src: u32) {
    testcase_init_from_to(dst, src)
        .expect_func_instrs([
            Instruction::memory_init_from_to(u32imm16(dst), u32imm16(src), Reg::from_i16(0)),
            Instruction::data_idx(0),
            Instruction::Return,
        ])
        .run()
...
fn test_copy_from_to(dst: u32, src: u32) {
    testcase_init_from_to(dst, src)
        .expect_func(
            ExpectedFunc::new([
                Instruction::memory_init(Reg::from_i16(-1), Reg::from_i16(-2), Reg::from_i16(0)),
                Instruction::data_idx(0),
                Instruction::Return,
            ])
            .consts([dst, src]),
...
fn test_copy_to_exact16(dst: u32, len: u32) {
    testcase_init_to_exact(dst, len)
        .expect_func_instrs([
            Instruction::memory_init_to_exact(u32imm16(dst), Reg::from_i16(0), u32imm16(len)),
            Instruction::data_idx(0),
            Instruction::Return,
        ])
        .run()
...
fn test_copy_to_exact(dst: u32, len: u32) {
    testcase_init_to_exact(dst, len)
        .expect_func(
            ExpectedFunc::new([
                Instruction::memory_init(Reg::from_i16(-1), Reg::from_i16(0), Reg::from_i16(-2)),
                Instruction::data_idx(0),
                Instruction::Return,
            ])
            .consts([dst, len]),
...
fn test_copy_from_exact16(src: u32, len: u32) {
    testcase_init_from_exact(src, len)
        .expect_func_instrs([
            Instruction::memory_init_from_exact(Reg::from_i16(0), u32imm16(src), u32imm16(len)),
            Instruction::data_idx(0),
            Instruction::Return,
        ])
        .run()
...
fn test_copy_from_to_exact(dst: u32, src: u32, len: u32) {
    testcase_init_from_to_exact(dst, src, len)
        .expect_func(
            ExpectedFunc::new([
                Instruction::memory_init(Reg::from_i16(-1), Reg::from_i16(-2), Reg::from_i16(-3)),
                Instruction::data_idx(0),
                Instruction::Return,
            ])
            .consts([dst, src, len]),
