...
impl Executor<'_> {
    ...
    pub fn execute_table_get(&mut self, result: Reg, index: Reg) -> Result<(), Error> {
        let index: u64 = self.get_register_as(index);
        self.execute_table_get_impl(result, index)
    }

    /// Executes an [`Instruction::TableGetImm`].
    pub fn execute_table_get_imm(&mut self, result: Reg, index: Const32<u64>) -> Result<(), Error> {
        let index: u64 = index.into();
        self.execute_table_get_impl(result, index)
    }

    /// Executes a `table.get` instruction generically.
    fn execute_table_get_impl(&mut self, result: Reg, index: u64) -> Result<(), Error> {
    ...
    pub fn execute_table_size(&mut self, result: Reg, table_index: Table) {
        self.execute_table_size_impl(result, table_index);
        self.next_instr();
    }

    /// Executes a generic `table.size` instruction.
    fn execute_table_size_impl(&mut self, result: Reg, table_index: Table) {
        let table = self.get_table(table_index);
        let size = self.store.inner().resolve_table(&table).size();
        self.set_register(result, size);
    }

    /// Executes an [`Instruction::TableSet`].
    pub fn execute_table_set(&mut self, index: Reg, value: Reg) -> Result<(), Error> {
        let index: u64 = self.get_register_as(index);
        self.execute_table_set_impl(index, value)
    }

    /// Executes an [`Instruction::TableSetAt`].
    pub fn execute_table_set_at(&mut self, index: Const32<u64>, value: Reg) -> Result<(), Error> {
        let index: u64 = index.into();
        self.execute_table_set_impl(index, value)
    }

    /// Executes a generic `table.set` instruction.
    fn execute_table_set_impl(&mut self, index: u64, value: Reg) -> Result<(), Error> {
        let table_index = self.fetch_table_index(1);
        let table = self.get_table(table_index);
    ...
    pub fn execute_table_copy(&mut self, dst: Reg, src: Reg, len: Reg) -> Result<(), Error> {
    ...
    fn execute_table_copy_impl(
        ...
        if dst_table_index == src_table_index {
            ...
            let (table, fuel) = self.store.inner_mut().resolve_table_and_fuel_mut(&table);
            ...
            let (dst_table, src_table, fuel) = self
                .store
                .inner_mut()
    ...
    pub fn execute_table_init(&mut self, dst: Reg, src: Reg, len: Reg) -> Result<(), Error> {
        let dst: u64 = self.get_register_as(dst);
    ...
    fn execute_table_init_impl(
        ...
        let table = self.get_table(table_index);
        let element = self.get_element_segment(element_index);
        let (table, element, fuel) = self
            .store
            .inner_mut()
            .resolve_table_init_params(&table, &element);
        table.init(element, dst_index, src_index, len, Some(fuel))?;
        self.try_next_instr_at(3)
    }

    /// Executes an [`Instruction::TableFill`].
    pub fn execute_table_fill(&mut self, dst: Reg, len: Reg, value: Reg) -> Result<(), Error> {
        let dst: u64 = self.get_register_as(dst);
        let len: u64 = self.get_register_as(len);
    ...
    #[inline(never)]
    fn execute_table_fill_impl(&mut self, dst: u64, len: u64, value: Reg) -> Result<(), Error> {
        let table_index = self.fetch_table_index(1);
        let value = self.get_register(value);
        let table = self.get_table(table_index);
        let (table, fuel) = self.store.inner_mut().resolve_table_and_fuel_mut(&table);
    ...
    pub fn execute_table_grow(&mut self, result: Reg, delta: Reg, value: Reg) -> Result<(), Error> {
        let delta: u64 = self.get_register_as(delta);
        self.execute_table_grow_impl(result, delta, value)
    }
    ...
    fn execute_table_grow_impl(
        ...
        let return_value = table.grow_untyped(delta, value, Some(fuel), &mut resource_limiter);
    ...
    pub fn execute_element_drop(&mut self, segment_index: Elem) {
