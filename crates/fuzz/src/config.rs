...
impl Arbitrary<'_> for FuzzSmithConfig {
    fn arbitrary(u: &mut Unstructured) -> arbitrary::Result<Self> {
        /// This limits the max number of simple Wasm entities.
        ///
        /// # Note
        ///
        /// This affects Wasm types, globals and funcs.
        const MAX_ENTITIES: usize = 1_000;

        /// This limits the max number of heap allocating Wasm entities.
        ///
        /// # Note
        ///
        /// This affects Wasm linear memories and tables.
        const MAX_DYN_ENTITIES: usize = 100;

        /// This limits the maximum size of linear memories and tables.
        ///
        /// # Note
        ///
        /// We limit this to avoid out-of-memory issues.
        const MAX_BUFFERS: u64 = 1_000_000;

        let config = wasm_smith::Config {
            max_types: u.int_in_range(0..=MAX_ENTITIES)?,
            max_imports: u.int_in_range(0..=MAX_ENTITIES)?,
            max_tags: 0, // Wasm `exceptions` proposal
            max_funcs: u.int_in_range(0..=MAX_ENTITIES)?,
            max_globals: u.int_in_range(0..=MAX_ENTITIES)?,
            max_exports: u.int_in_range(0..=MAX_ENTITIES)?,
            max_element_segments: u.int_in_range(0..=MAX_ENTITIES)?,
            max_elements: u.int_in_range(0..=MAX_ENTITIES)?,
            max_data_segments: u.int_in_range(0..=MAX_ENTITIES)?,
            max_instructions: u.int_in_range(0..=MAX_ENTITIES)?,
            max_memories: u.int_in_range(0..=MAX_DYN_ENTITIES)?,
            max_tables: u.int_in_range(0..=MAX_DYN_ENTITIES)?,
            max_memory32_bytes: u.int_in_range(0..=MAX_BUFFERS)?,
            max_memory64_bytes: u.int_in_range(0..=u128::from(MAX_BUFFERS))?,
            min_uleb_size: u.int_in_range(0..=5)?,
            max_table_elements: u.int_in_range(0..=MAX_BUFFERS)?,
...
impl FuzzSmithConfig {
    /// Allows to execute Wasm blobs generated by this [`FuzzSmithConfig`].
    ///
    /// This means that some limits need to be reduced to avoid out-of-memory issues.
    pub fn allow_execution(&mut self) {
        fn clamp_max<T: Ord + Copy>(this: &mut T, max: T) {
            *this = (*this).min(max);
        }
        let Self { inner } = self;
        clamp_max(&mut inner.max_table_elements, 100_000);
        clamp_max(&mut inner.max_memory32_bytes, 1_000_000);
        clamp_max(&mut inner.max_memory64_bytes, 1_000_000);
    }
